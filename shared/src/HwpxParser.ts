...
export class HwpxParser {
  ...
  static async parse(zip: JSZip): Promise<HwpxContent> {
    ...
    // Reset static styles for next parse call
    this.styles = {
      charShapes: new Map(),
      paraShapes: new Map(),
      fonts: new Map(),
  ...
  private static parseSection(xml: string, content: HwpxContent): HwpxSection {
    ...
    // Also parse simplified <hp:memo> elements (written by insertMemo)
    ...
    while ((simpleMemoMatch = simpleMemoRegex.exec(xml)) !== null) {
      ...
      // Skip if already parsed via fieldBegin
    ...
    // Also remove simplified <hp:memo> elements to prevent memo text from appearing as document content
    ...
    for (const el of elements) {
      if (el.type === 'p') {
        // Skip pic-only and equation-only host paragraphs (no visible text content)
        // to match HWP parser behavior which discards such host paragraphs.
        const containsPic = /<hp:pic\b/.test(el.xml);
        const containsEquation = /<hp:equation\b/.test(el.xml);
        if (containsPic || containsEquation) {
          const strippedXml = el.xml
            .replace(/<hp:pic\b[\s\S]*?<\/hp:pic>/g, '')
            .replace(/<hp:equation\b[\s\S]*?<\/hp:equation>/g, '');
        ...
        for (const fnRef of footnoteRefPositions) {
          ...
          if (fnRef.position >= el.index && fnRef.position < el.index + el.xml.length + 500) {
            ...
            // inheriting char style from the paragraph's existing run.
            // Keep the base fontSize (superscript flag handles visual scaling)
            // so roundtrip through HWP binary preserves the char shape.
            const baseCharStyle = paragraph.runs.length > 0
              ? paragraph.runs[paragraph.runs.length - 1].charStyle
              : undefined;
            paragraph.runs.push({
              text: `${fnRef.number})`,
              footnoteRef: fnRef.type === 'footnote' ? fnRef.number : undefined,
              endnoteRef: fnRef.type === 'endnote' ? fnRef.number : undefined,
              charStyle: { ...baseCharStyle, superscript: true },
            });
  ...
  private static parseHeaderFooter(xml: string, type: 'header' | 'footer'): import('./types').HeaderFooter | undefined {
    ...
    if (elements.length === 0 && paragraphs.length === 0) return undefined;
  ...
  private static parseColumnDef(xml: string): import('./types').ColumnDef | undefined {
    ...
    const typeMatch = content.match(/type="([^"]*)"/);
    if (typeMatch) {
      const typeMap: Record<string, import('./types').ColumnType> = { 'newspaper': 'Newspaper', 'balancednewspaper': 'BalancedNewspaper', 'parallel': 'Parallel' };
      colDef.type = typeMap[typeMatch[1].toLowerCase()] || 'Newspaper';
    }
    ...
    if (layoutMatch) {
      const layoutMap: Record<string, import('./types').ColumnLayout> = { 'left': 'Left', 'right': 'Right', 'mirror': 'Mirror' };
      colDef.layout = layoutMap[layoutMatch[1].toLowerCase()] || 'Left';
    }
  ...
  private static parseRun(xml: string): TextRun[] {
    ...
    // Also check for memoId attribute on the <hp:run> tag itself (written by insertMemo)
    if (!memoId) {
  ...
  private static parseTable(xml: string, content?: HwpxContent): HwpxTable {
    ...
    // Fill in missing cell widths from columnWidths or table width
    const colCount = table.rows[0]?.cells.length || 1;
    const tableWidthPt = table.width || 425.2;
    const hasColWidths = table.columnWidths && table.columnWidths.length > 0;
    for (const row of table.rows) {
      for (const cell of row.cells) {
        if (cell.width === undefined || isNaN(cell.width)) {
          const colAddr = cell.colAddr ?? 0;
          const colSpan = cell.colSpan ?? 1;
          if (hasColWidths) {
            let w = 0;
            for (let ci = colAddr; ci < colAddr + colSpan && ci < table.columnWidths!.length; ci++) {
              w += table.columnWidths![ci];
            }
            if (w > 0) { cell.width = w; continue; }
          }
          // Fallback: calculate from table width evenly
          cell.width = (tableWidthPt / colCount) * colSpan;
        }
      }
    }
    ...
    if (caption) {
      // Extract caption text from paragraphs
      const captionText = caption.paragraphs?.map(p => p.runs?.map(r => r.text).join('')).join('\n') || '';
      if (captionText) table.caption = captionText;
      if (caption.side) table.captionPosition = (caption.side === 'Top') ? 'above' : 'below';
    }
  ...
  private static extractBalancedTags(xml: string, tagName: string): { xml: string; start: number; end: number }[] {
    ...
    // Verify the character after the prefix is a tag delimiter (>, space, /, tab, newline, CR)
    // to avoid false matches like <hp:p matching <hp:pic, <hp:pos, etc.
  ...
  private static parseCellContent(contentXml: string, cell: TableCell, content?: HwpxContent): void {
    ...
    // Extract images (hp:pic) from cell content for caption text extraction
    ...
    if (nestedTables.length > 0 || nestedImages.length > 0) {
      ...
      // Extract all paragraphs (including ones that contain tables/images)
      const allParagraphs = this.extractAllParagraphs(cleanedXml);

      // Build ordered elements: paragraphs, tables, and images in document order
      const elements: { index: number; type: 'p' | 'tbl' | 'pic'; xml: string }[] = [];

      for (const para of allParagraphs) {
        const isInsideExcluded = excludeRanges.some(
          range => para.start > range.start && para.start < range.end
        );
        if (isInsideExcluded) continue; // Skip paragraphs inside nested tables/images

        const containsExcluded = excludeRanges.some(
          range => range.start >= para.start && range.end <= para.end
        );
        if (containsExcluded) {
          // Paragraph contains a table/image - strip excluded XML and parse the remaining text
  ...
  private static parseCaption(xml: string): import('./types').Caption | undefined {
    ...
    for (const pBlock of pBlocks) {
      const para = this.parseParagraph(pBlock.xml);
      if (para.runs.length > 0) {
        paragraphs.push(para);
      }
    }

    // Also extract text from nested shape elements within this caption
    ...
    for (const shapeName of ['hp:pic', 'hp:tbl']) {
      ...
      for (const shapeBlock of nestedShapes) {
        const nestedCaption = this.parseCaption(shapeBlock.xml);
    ...
    return caption.paragraphs ? caption : undefined;
  }
  ...
  private static parseContainer(xml: string, content: HwpxContent): HwpxContainer {
    ...
    // Parse caption (store on container if type supports it)
    const caption = this.parseCaption(xml);
    if (caption) {
      (container as any).caption = caption;
    }
