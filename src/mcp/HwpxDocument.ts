...
export class HwpxDocument {
  ...
  public static createNew(id: string, title?: string, creator?: string): HwpxDocument {
    ...
    const content: HwpxContent = {
      ...
      endnotes: [],
      styles: {
        styles: new Map([[0, { name: '본문', type: 'paragraph', paraPrIdRef: 0, charPrIdRef: 0 }]]),
        charShapes: new Map([[0, { height: 1000 }]]),
        paraShapes: new Map([[0, { align: 'left' }]]),
      },
    };
...
<hp:sec xmlns:hp="http://www.hancom.co.kr/hwpml/2011/paragraph">
  ...
  private extractTableText(table: HwpxTable): string {
  ...
  private async syncContentToZip(): Promise<void> {
    ...
    const hasImageChanges = (this._pendingImageInserts && this._pendingImageInserts.length > 0)
      || (this._pendingImageDeletes && this._pendingImageDeletes.length > 0)
      || (this._pendingImageSizeUpdates && this._pendingImageSizeUpdates.length > 0);

    // Handle structural changes (insertMemo, deleteMemo, etc.) - but not image ops
    if (this._hasStructuralChanges && !hasImageChanges) {
      await this.syncStructuralChangesToZip();
      this._hasStructuralChanges = false;
    }

    // Apply image changes via XML injection (preserves existing XML)
    if (hasImageChanges) {
  ...
  private async applyImageChangesToXml(): Promise<void> {
    ...
    for (const sectionIndex of affectedSections) {
      ...
      for (const del of this._pendingImageDeletes.filter(d => d.sectionIndex === sectionIndex)) {
        ...
        // Try 1: Remove entire <hp:p> wrapping the image (simple structure)
        ...
        // Try 2: Remove just the <hp:pic>...</hp:pic> element (nested cases)
  ...
  /**
   * Sync structural changes by regenerating section XML from in-memory content.
   */
    ...
    if (table.rows) {
      for (const row of table.rows) {
        ...
        for (const cell of row.cells) {
          ...
          for (const para of (cell.paragraphs || [])) {
            xml += this.generateParagraphXml(para, 8);
          }
          xml += `      </hp:tc>\n`;
        }
        xml += `    </hp:tr>\n`;
  ...
  private async syncBinDataToHpf(): Promise<void> {
    ...
    hpfXml = hpfXml.replace(removeRegex, (fullMatch: string, itemId: string) => {
