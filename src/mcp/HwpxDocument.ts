...
<hp:sec xmlns:hp="http://www.hancom.co.kr/hwpml/2011/paragraph">
   ...
   * Based on extension's updateZip logic.
   */
  private async syncStructuralChangesToZip(): Promise<void> {
    ...
    for (let sectionIndex = 0; sectionIndex < this._content.sections.length; sectionIndex++) {
      const sectionPath = `Contents/section${sectionIndex}.xml`;
      const existingXml = await this._zip.file(sectionPath)?.async('string');

      if (existingXml) {
        const updatedXml = this.updateSectionXml(existingXml, this._content.sections[sectionIndex]);
        this._zip.file(sectionPath, updatedXml);
      }
    }
